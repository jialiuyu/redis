 ...
 *
 * Enhanced with UB (Unified Bus) support for high-performance feature queries
 * using SVE (Scalable Vector Extension) acceleration.
 *
...
static int vector_engine_enabled = 0;
static vector_engine_type_t current_engine_type = VECTOR_ENGINE_REDIS;
 ...
 *
 * Enhanced with UB engine support for high-performance embedding queries.
 */
...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    // Register command VENGINE - Vector Engine Control
    if (RedisModule_CreateCommand(ctx, "VENGINE",
        VENGINE_RedisCommand, "readonly", 0, 0, 0) == REDISMODULE_ERR)
        return REDISMODULE_ERR;
