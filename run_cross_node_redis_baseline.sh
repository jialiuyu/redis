#!/bin/bash
# 注意：不能用 set -e —— vanilla redis VEMB 读路径在高并发下偶发 connection reset，
# 一档失败会让整个脚本退出触发 cleanup 杀 server，后续档全挂。用 || true 兜底。

# ============================================================================
# Cross-Node VEMB Baseline Benchmark (native Redis 8.6.3 + memtier_benchmark)
# Topology is parameterized via env vars (defaults shown below):
#   JUMP=HW01      — jump host orchestrating the run via nested ssh
#   SERVER=HW01    — redis-server 8.6.3 baseline (RESP VECTOREMB path)
#   CLIENT=HW03    — memtier_benchmark_origin (TCP client)
# Override per-run, e.g.:
#   SERVER=HW04 CLIENT=HW06 SERVER_HOST=192.168.90.21 ./run_cross_node_redis_baseline.sh
# ============================================================================

# Topology — override via env vars, e.g.
#   SERVER=HW04 CLIENT=HW06 SERVER_HOST=192.168.90.21 ./run_cross_node_redis_baseline.sh
# Defaults: HW01 server + HW03 client (original config)
JUMP="${JUMP:-HW01}"
SERVER="${SERVER:-HW01}"
#CLIENT="${CLIENT:-HW03}"
CLIENT="${CLIENT:-192.168.1.112}"
...
SERVER_LOG="${SERVER_LOG:-/tmp/redis_vemb_baseline_server.log}"

TEST_TIME=${TEST_TIME:-60}
# t×c ≤ 4000：c=20, threads=20/40/80/120/200 → 400/800/1600/2400/4000
# c=20：sweep 数据确认 vanilla redis baseline 在 c≤64 稳定；c=100 时 400 连接 19s 后崩。
...
RESULT_PREFIX="vemb_cross_baseline_${TIMESTAMP}"
...
cleanup_client() {
    ...
    ssh "$JUMP" "ssh $CLIENT \"pkill memtier_bench 2>/dev/null || true; pkill memtier_benchmark 2>/dev/null || true; rm -f /tmp/vemb_cross_baseline_bench.sh /tmp/vemb_cross_baseline_*.log 2>/dev/null || true\"" || true
}
...
# ============================================================================
# Step 1: Start hpc-redis-server on $SERVER
# ============================================================================
...
ssh "$JUMP" "ssh $SERVER bash -s" << REMOTE_EOF
    ...
    # Wait for the old server to fully release port 7001 (it does RDB save
    # on SIGTERM before exiting). Without this, the new server hits
    # "bind: Address already in use" and aborts.
    ...
    numactl --membind=0 taskset -c 0-95 ./src/redis-server \
        ...
        --tcp-keepalive 1800 \
        --timeout 0 \
        --io-threads ${IO_THREADS:-16} --io-threads-do-reads yes \
        --daemonize yes \
        --logfile $SERVER_LOG
    sleep 2
...
# ============================================================================
# Step 3: Run memtier benchmark on $CLIENT
# ============================================================================
log "=== Step 3: Run memtier benchmark on $CLIENT ==="
...
BENCH_SCRIPT_NAME="vemb_cross_baseline_bench.sh"
BENCH_SCRIPT_JUMP="/tmp/$BENCH_SCRIPT_NAME"

ssh "$JUMP" "cat > $BENCH_SCRIPT_JUMP" <<'BENCH_SCRIPT'
#!/bin/bash
# 不用 set -e：vanilla redis VEMB 高并发下偶发 connection reset，memtier 非零退出
# 是正常的，不能让一档失败中断整轮。
threads=$1
clients=$2
test_time=$3
...
# Upload jiffies helper to JUMP then scp to SERVER（sum utime+stime across all TIDs）
JIFFIES_HELPER="/tmp/get_remote_jiffies.sh"
ssh "$JUMP" "cat > $JIFFIES_HELPER" <<'JIFFIES_EOF'
#!/bin/bash
pid=$1
s=0
...
echo $s
JIFFIES_EOF
ssh "$JUMP" "scp $JIFFIES_HELPER $SERVER:/tmp/get_remote_jiffies.sh" 2>/dev/null

for threads in "${THREADS_LIST[@]}"; do
    log "--- threads=$threads clients=$CLIENTS_PER_THREAD pipeline=$PIPELINE time=${TEST_TIME}s ---"
    ...
    # Totals 行优先（有完整 latency 百分位）；缺失时（connection reset / 非零退出）
    # 从 progress 行提取 ops/sec + avg_lat + KB/sec（percentile 百分位此时不可用）。
    ...
    cores=$(awk "BEGIN{ if(\"${J0:-}\"==\"\" || \"${J1:-}\"==\"\") print \"NA\"; else printf \"%.2f\", (${J1:-0}-${J0:-0})/100.0/${TEST_TIME} }")
    ...
    log "  t$threads: ops/sec=$ops avg_lat=$avg p99=$p99 p99.9=$p999 KB/sec=$kb cores=$cores base=${BASE_MB:-N/A}MB peak=${PEAK_MB:-N/A}MB"
done
...
log "=== Results Summary ==="
{
    printf "%-8s %12s %12s %12s %12s %12s %12s %8s %10s %10s %10s\n" "threads" "ops/sec" "avg_lat" "p99_lat" "p99.9_lat" "KB/sec" "run_time(s)" "cores" "base_MB" "peak_MB" "avg_MB"
    for i in "${!OPS_SEC[@]}"; do
        printf "%-8s %12s %12s %12s %12s %12s %12s %8s %10s %10s %10s\n" \
            "${THREAD_VAL[$i]}" \
            "${OPS_SEC[$i]:-N/A}" \
