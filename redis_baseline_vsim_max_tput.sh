#!/bin/bash
# vanilla redis 8.6.3 VSIM max-tput sweep：io-threads N + memtier -t×-c≤64
#
#   对照 hpc_redis_vsim_max_tput.sh，看 vanilla redis HNSW VSIM 跟
#   hpc_redis mmap+SIMD VSIM 的吞吐差。同样 10000 向量 prefill, 100% VSIM 读。
#
#   VSIM 走 RESP 自定义命令：memtier --command="VSIM myset ELE __key__"
#   vanilla redis 8.6.3 内置 vector-sets 模块（VSIM/VADD 是 module 命令，
#   VSIM 默认 threaded，跑在 HNSW 自己的线程池里）
#
# 用法: PORT=6391 ./redis_baseline_vsim_max_tput.sh
#   smoke: NS="8" TS="16" CS="16" TEST_TIME=3 ./redis_baseline_vsim_max_tput.sh
set -uo pipefail
...
# memtier -t values — redis baseline: 聚焦可信区（单连接线程扩展 + 多连接扩展 + 旧甜点 t32c1 验证）
TS=( ${TS:-1 1 1 4 8 16 32 32 32} )
# memtier -c values (paired with TS; t×c ≤ 256, 避开高并发过载崩溃区)
CS=( ${CS:-1 64 200 64 32 16 8 2 1} )

OUTDIR=${OUTDIR:-/tmp/redis_baseline_vsim_max_tput}
RAWDIR=$OUTDIR/raw
mkdir -p "$RAWDIR"
TSV=$OUTDIR/summary.tsv
PIDFILE=/tmp/redis_vsim_baseline.pid
VADD_PIPE=/tmp/vadd_pipe.txt
...
run_client() {
    ...
    mem_samples_file=/tmp/mem_samples_redis_vsim_${tag}.log
    ...
    printf '   N=%-2s t%-2s c%-2s  ops=%-12s p50=%-8s cores=%-5s  mem=%s/%sMB\n' \
        "$n" "$t" "$c" "$eff" "$p50" "$cores" "$mem_base_mb" "$mem_peak_mb"
}

log "vanilla redis VSIM max-tput sweep — ${#NS[@]} N × ${#TS[@]} memtier pts = $(( ${#NS[@]}*${#TS[@]} )) runs"
gen_vadd_pipe
for n in "${NS[@]}"; do
