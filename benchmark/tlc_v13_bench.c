/*
 * TLC V13 Benchmark — Zero-Copy Aeron IPC
 *
...
int main(int argc, char *argv[]) {
    ...
    printf("╔═══════════════════════════════════════════════════════════════════╗\n");
    printf("║  V13 Benchmark: Zero-Copy Aeron IPC vs V9 Aeron IPC             ║\n");
    printf("║  Ops: %zu  Threads: %d  Value: %dB                             ║\n", ops, threads, VALUE_SIZE);
    printf("╚═══════════════════════════════════════════════════════════════════╝\n\n");

    /* Fill v10 server */
    fill_uds("/tmp/tlc_v13.sock");

    printf("\n  %-45s %9s %6s %5s\n", "Transport", "QPS", "M/s", "Lat");
    printf("  %-45s %9s %6s %5s\n", "---------------------------------------------", "---------", "------", "-----");

    /* V13 Aeron IPC (zero-copy GET: 5B response) */
    run("V13 Aeron 80R/20W (zero-copy, 8 ch)", ops, threads, 20, mk, "/tmp/tlc_v13.sock", "tlc_v13");
    run("V13 Aeron 100%% GET (zero-copy)", ops, threads, 0, mk, "/tmp/tlc_v13.sock", "tlc_v13");
    run("V13 Aeron 100%% PUT", ops, threads, 100, mk, "/tmp/tlc_v13.sock", "tlc_v13");
