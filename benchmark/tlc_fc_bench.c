/*
 * TLC FC Benchmark — Zero-Copy Aeron IPC
 *
...
int main(int argc, char *argv[]) {
    ...
    printf("╔═══════════════════════════════════════════════════════════════════╗\n");
    printf("║  FC Benchmark: Zero-Copy Aeron IPC vs V9 Aeron IPC             ║\n");
    printf("║  Ops: %zu  Threads: %d  Value: %dB                             ║\n", ops, threads, VALUE_SIZE);
    printf("╚═══════════════════════════════════════════════════════════════════╝\n\n");

    /* Fill v10 server */
    fill_uds("/tmp/tlc_fc.sock");

    printf("\n  %-45s %9s %6s %5s\n", "Transport", "QPS", "M/s", "Lat");
    printf("  %-45s %9s %6s %5s\n", "---------------------------------------------", "---------", "------", "-----");

    /* FC Aeron IPC (zero-copy GET: 5B response) */
    run("FC Aeron 80R/20W (zero-copy, 8 ch)", ops, threads, 20, mk, "/tmp/tlc_fc.sock", "tlc_fc");
    run("FC Aeron 100%% GET (zero-copy)", ops, threads, 0, mk, "/tmp/tlc_fc.sock", "tlc_fc");
    run("FC Aeron 100%% PUT", ops, threads, 100, mk, "/tmp/tlc_fc.sock", "tlc_fc");
