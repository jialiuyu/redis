#!/bin/bash
# vanilla redis 8.6.3 VADD 单 client pipeline 扫描（方法论 A）
#
#   -t 1 -c 1 -n FIXED_N，扫 pipeline。每个 pipeline 点不重叠 key 区间（分片），插入
#   全新 key。VADD 不需要 prefill（它本身就是插入）；每个 N 重启 server + DEL myset
#   保证干净起点。
#
#   VADD 走 RESP module 命令，"write" flag 主线程串行执行（跟 VREM 一样）。HNSW 插入
#   要搜插入点 + 算距离，比 VREM 删除重，预期吞吐 < VREM。
#
#   memtier --command 无法动态生成随机 vector，用固定 300-dim vector（awk 固定种子
#   生成一次）。vector 值对吞吐影响小（距离计算量恒定 300-dim），文档会注明这个
#   hpc(随机) vs baseline(固定) 的不对称。
#
#   FIXED_N 默认 10000；6 点分片共 60K 插入（HNSW 从 0 增长到 60K，后期插入略慢）
#
# 用法: PORT=6391 ./redis_baseline_vadd_single_client.sh
#   smoke: NS="8" PIPELINES="32" FIXED_N=2000 ./redis_baseline_vadd_single_client.sh
set -uo pipefail
...
OUTDIR=/tmp/redis_baseline_vadd_single
RAWDIR=$OUTDIR/raw
mkdir -p "$RAWDIR"
TSV=$OUTDIR/summary.tsv
PIDFILE=/tmp/redis_vadd_single.pid
...
log "vanilla redis VADD single-client sweep — ${#NS[@]} N × ${#PIPELINES[@]} pipelines = $(( ${#NS[@]}*${#PIPELINES[@]} )) runs (FIXED_N=$FIXED_N, 无 prefill, 分片插入新 key, 固定 vector)"
for n in "${NS[@]}"; do
    log "N=$n — starting server + DEL myset (干净起点)"
    ...
    for pl in "${PIPELINES[@]}"; do
        ...
        t0=$(date +%s.%N)
        timeout 300s $CLIENT_BIND $MEMTIER --command="VADD myset VALUES 300 $FIXED_VECTOR __key__" --command-key-pattern=S \
