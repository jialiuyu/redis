...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "--socket") && i + 1 < argc) {
            ...
            printf("usage: %s [--socket PATH] [--dim N] [--max-vectors N]\n", argv[0]);
            return 0;
        }
    }
    ...
    if (vemb_v16_proxy_create(&g_proxy, uds_path, dim, max_vectors) != 0) {
        fprintf(stderr, "failed to create vemb_v16 proxy\n");
        return 1;
    }
    ...
    printf("\n=== vemb_v16 stats ===\n");
    printf("total=%llu vadd=%llu vemb=%llu not_found=%llu published=%llu completed=%llu active_channels=%llu\n",
    ...
    printf("bitmap_lock_success=%llu bitmap_lock_failure=%llu sample_vector_load_ns=%llu\n",
