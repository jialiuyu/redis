...
#include "vemb_v16_proxy_thread.h"
...
static void vemb_v16_channel_free_slots(vemb_v16_channel_t *ch) {
    if (!ch) return;
    zfree(ch->vemb_job_slots);
    zfree(ch->vadd_job_slots);
    zfree(ch->completion_slots);
...
/* ============================================================
 * Threaded proxy launcher
 * ============================================================ */

static void *vemb_v16_proxy_thread_entry(void *arg) {
    vemb_v16_proxy_thread_result_t *res = arg;
    res->exit_code = vemb_v16_proxy_run(res->proxy);
    return res;
}

int vemb_v16_proxy_run_in_thread(vemb_v16_proxy_t *proxy,
                                 vemb_v16_proxy_thread_result_t **out_result) {
    if (!proxy || !out_result) return -1;
    vemb_v16_proxy_thread_result_t *res = zmalloc(sizeof(*res));
    if (!res) return -1;
    res->proxy = proxy;
    res->exit_code = -1;

    pthread_t tid;
    if (pthread_create(&tid, NULL, vemb_v16_proxy_thread_entry, res) != 0) {
        zfree(res);
        return -1;
    }
    pthread_detach(tid);
...
void vemb_v16_proxy_thread_wait(vemb_v16_proxy_thread_result_t *res) {
    if (!res) return;
    zfree(res);
}
