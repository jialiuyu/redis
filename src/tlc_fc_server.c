/*
 * TLC Flat Combining Server — Adaptive V12/Batch Hybrid
 *
...
#define UDS_PATH       "/tmp/tlc_fc.sock"
#define MAX_SLOTS      64
    ...
    if (became_combiner) {
        ...
        if (pending_count > 0) {
            ...
            for (int p = 0; p < pending_count && p < 8; p++)
                __builtin_prefetch(&g_cache.hot.table[hash_pf(pending_keys[p], hot_mask)], 0, 3);

            for (int i = 0; i < pending_count; i++) {
                if (i + 8 < pending_count)
                    __builtin_prefetch(&g_cache.hot.table[hash_pf(pending_keys[i+8], hot_mask)], 0, 3);
...
static int create_channel(void) {
    ...
    ch->id = id;
    char name[64];
    snprintf(name, sizeof(name), "/tlc_fc_req_%d", id);
    ...
    snprintf(name, sizeof(name), "/tlc_fc_resp_%d", id);
...
int main(void) {
    ...
    printf("╔═══════════════════════════════════════════════════════════════╗\n");
    printf("║  TLC Flat Combining Server                                   ║\n");
