...
object_generator::object_generator(const object_generator& copy) :
    ...
    m_key_zipfian_s(copy.m_key_zipfian_s),
    m_value_buffer(NULL),
    m_random_fd(-1),
    m_value_buffer_size(0),
    m_value_buffer_mutation_pos(0)
...
unsigned long long object_generator::get_key_index(int iter)
{
    ...
    if (iter==OBJECT_GENERATOR_KEY_RANDOM) {
        ...
        k = zipfian_distribution(m_key_min, m_key_max, m_key_zipfian_s);
    } else {
...
void object_generator::set_vector_dim(uint32_t dim)
{
    m_vector_dim = dim;
    if (dim > 0)
        alloc_vector_buffer();
...
void object_generator::alloc_vector_buffer(void)
{
    if (m_vector_buffer != NULL)
        free(m_vector_buffer), m_vector_buffer = NULL;

    if (m_vector_dim == 0)
        return;

    m_vector_buffer = (float*)malloc(m_vector_dim * sizeof(float));
    assert(m_vector_buffer != NULL);

    for (uint32_t i = 0; i < m_vector_dim; i++) {
        // Generate random float in range [-1.0, 1.0]
        m_vector_buffer[i] = ((float)(m_random.get_random() % 2000000) / 1000000.0f) - 1.0f;
    }
}

const char* object_generator::get_value(unsigned long long key_index, unsigned int *len) {
    // VEMB vector mode
    if (m_vector_dim > 0 && m_vector_buffer != NULL) {
        *len = m_vector_dim * sizeof(float);
        return (const char*)m_vector_buffer;
    }
