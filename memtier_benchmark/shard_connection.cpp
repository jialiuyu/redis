...
int shard_connection::connect(struct connect_info* addr) {
    ...
    m_hello = (m_config->protocol == PROTOCOL_RESP2 || m_config->protocol == PROTOCOL_RESP3 || m_config->protocol == PROTOCOL_VEMB_V16) ? setup_none : setup_done;
...
void shard_connection::send_conn_setup_commands(struct timeval timestamp) {
    ...
    if (m_hello == setup_none) {
        ...
        if (m_config->protocol == PROTOCOL_VEMB_V16) {
            ((vemb_v16_protocol*)m_protocol)->write_hello();
        } else {
...
void shard_connection::process_response(void)
{
    ...
    // VEMB V16: handle WELCOME response before normal response loop
    if (m_config->protocol == PROTOCOL_VEMB_V16 && m_hello == setup_sent) {
        int welcome_ret = ((vemb_v16_protocol*)m_protocol)->parse_welcome();
        if (welcome_ret > 0) {
            m_hello = setup_done;
            request* req = pop_req();
            delete req;
        } else if (welcome_ret < 0) {
            benchmark_error_log("error: VEMB WELCOME parsing failed, disconnecting.\n");
            disconnect();
            return;
        } else {
            // WELCOME not fully received yet
            return;
        }
    }
    ...
    if (ret == -1) {
        benchmark_error_log("error: response parsing failed, disconnecting.\n");
        disconnect();
        return;
    }
