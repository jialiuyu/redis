...
static const char * get_protocol_name(enum PROTOCOL_TYPE type) {
    ...
    else if (type == PROTOCOL_VEMB_V16) return "vemb_v16";
    else return "none";
}
...
static int config_parse_args(int argc, char *argv[], struct benchmark_config *cfg)
{
    ...
    static struct option long_options[] = {
        ...
        { "key-zipfian-s",            1, 0, o_key_zipfian_s },
                ...
                "vs:S:p:P:o:x:DRn:c:t:d:a:h:46", long_options, &option_index)) != -1)
    {
        switch (c) {
                ...
                case 'P':
                    ...
                    } else if (strcmp(optarg, "vemb_v16") == 0) {
                        cfg->protocol = PROTOCOL_VEMB_V16;
                    } else {
                ...
                case o_key_median:
                    ...
                    break;
                case o_key_zipfian_s:
                    endptr = NULL;
                    cfg->key_zipfian_s = strtod(optarg, &endptr);
                    if (cfg->key_zipfian_s <= 0 || !endptr || *endptr != '\0') {
                        fprintf(stderr, "error: key-zipfian-s must be greater than zero.\n");
                        return -1;
                    }
                    break;
                case o_key_pattern:
...
void usage() {
    fprintf(stdout, "Usage: memtier_benchmark [options]\n"
            ...
            "      --key-zipfian-s=S          The skew factor s for the Zipf distribution (use with --key-pattern=Z:Z)\n"
            "                                 (default is 0.99, 1.0 = classic Zipf, larger = more skew)\n"
            "\n"
            "VEMB V16 Options:\n"
...
// TLS 1.3 is only available as from version 1.1.1.
    ...
    if (cfg.key_pattern[key_pattern_set]=='Z' || cfg.key_pattern[key_pattern_get]=='Z') {
        if (cfg.key_zipfian_s <= 0)
            cfg.key_zipfian_s = 0.99;
        obj_gen->set_key_zipfian(cfg.key_zipfian_s);
    } else if (cfg.key_zipfian_s > 0) {
        fprintf(stderr, "error: key-zipfian-s is only allowed together with key-pattern set to Z.\n");
        usage();
    }
    if (cfg.protocol == PROTOCOL_VEMB_V16 && cfg.vemb_v16_dim > 0) {
        obj_gen->set_vector_dim(cfg.vemb_v16_dim);
    }
